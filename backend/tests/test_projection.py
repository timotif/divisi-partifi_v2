...
# Ground truth
# ---------------------------------------------------------------------------
# (filename, page_num, expected_systems, expected_staves_per_system)
#   filename: file inside tests/img/
...
CASES = [
    ...
    ("fantasia.pdf", 0, 4, [4, 4, 4, 4]),
    ("fantasia.pdf", 1, 4, [4, 4, 4, 4]),
    ("fantasia.pdf", 2, 4, [4, 4, 4, 4]),
    ("fantasia.pdf", 3, 5, [4, 4, 4, 4, 4]),
    ("fantasia.pdf", 4, 5, [4, 4, 4, 4, 4]),
    ("fantasia.pdf", 5, 4, [4, 4, 4, 4]),
    ("fantasia.pdf", 6, 4, [4, 4, 4, 4]),
    ("fantasia.pdf", 7, 4, [4, 4, 4, 4]),
    # schauspieldirektor — 2 systems × 12 staves
    ("schauspieldirektor.pdf",  4, 2, [12, 12]),
    ("schauspieldirektor.pdf",  9, 2, [12, 12]),
    ("schauspieldirektor.pdf", 11, 2, [12, 12]),
...
def _case_id(case):
    filename, page, _, _ = case
    name = filename.rsplit(".", 1)[0]
    return f"{name}_p{page}"


@pytest.mark.parametrize(
    "filename, page_num, expected_systems, expected_staves",
    CASES,
    ids=list(map(_case_id, CASES)),
)
def test_detection(filename, page_num, expected_systems, expected_staves):
